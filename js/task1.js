...
//============================================
//Задание 2
//Получить массив объектов пользователей по цвету 
//глаз (поле eyeColor).
...
const getUsersWithGender = (users, gender) => {
 const genderName = users.filter(user => user.gender === gender);
 const usersName = genderName.map(user => user.name);
 return usersName; 
};
...
const getUsersWithFriend = (users, friendName) => {
  const usersHaveFriends = users.filter(user => user.friends.includes(friendName));
  const usersName = usersHaveFriends.map(user => user.name);
  return usersName;
};
...
const getNamesSortedByFriendsCount = users => {
  const usersHaveFriends = users.sort((prev, next) => prev.friends.length - next.friends.length);
  const usersName = usersHaveFriends.map(user => user.name);
  return usersName;
};
...
const usersAllSkills = users
.flatMap(user => user.skills)
.filter((user, index, arr) => {
  return arr.indexOf(user) === index;
})
// .filter((user, _index, arr) => {
